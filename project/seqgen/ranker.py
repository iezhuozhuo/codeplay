...
class Ranker(nn.Module):
    def __init__(self, args, vocab_src, vocab_tgt, embed_dim, ff_embed_dim, num_heads, dropout, num_layers):
        ...
        self.embed_positions = SinusoidalPositionalEmbedding(embed_dim, device=args.device)
    ...
    def forward(self, src_input, tgt_input, work=False):
        ...
        if work:
            return src, tgt
        # bsz x bsz
        scores = torch.mm(self.scorer(src), tgt.transpose(0, 1))
        log_probs = F.log_softmax(scores, -1)

        gold = torch.arange(bsz).to(self.args.device)
