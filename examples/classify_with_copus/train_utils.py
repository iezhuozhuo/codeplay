...
class trainer(Trainer):
    ...
    def train_epoch(self):
        ...
        for batch_id, batch in enumerate(self.train_iter, 1):
            ...
            inputs_id = batch.src[0].to(self.args.device)
            inputs_label = batch.tgt.to(self.args.device)
...
def evaluate(args, model, valid_dataset, logger):
    ...
    for batch in valid_dataset:
        inputs_id = batch.src[0].to(args.device)
        inputs_label = batch.tgt.to(args.device)
        with torch.no_grad():
