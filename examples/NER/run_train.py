...
from preprocessing_zh import CNERCorpus
...
MODEL_CLASSES = {
    "rnncrf": (RNNCRFModel, RNNCRFConfig)
}
...
def main():
    ...
    processor = CNERCorpus(args=args, specials=specials)
    padding_idx = processor.field["text"].stoi[constants.PAD_WORD]
    args.id2label = processor.field["label"].id2label
    args.label2id = processor.field["label"].label2id

    embedded_pretrain = None
    if hasattr(args, "embed_file"):
