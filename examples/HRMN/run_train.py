...
def main():
    ...
    padding_idx = processor.field["text"].stoi["<PAD>"]
    ...
    # embedded_pretrain.load_embeddingsfor_gensim_vec_with_glove("/home/administrator4/ZDY/dataset/",
    #                                                            'glove.840B.300d.txt',
    #                                                            processor.field["text"].stoi,
    #                                                            processor.field["text"].itos)
    #
    # char_padding_idx = processor.field["char"].stoi[constants.PAD_WORD]
    # char_embedded_pretrain = Embedder(num_embeddings=processor.field["char"].vocab_size,
    #                              embedding_dim=args.char_embedding_dim, padding_idx=char_padding_idx)
    #
    # logger.info(args)
    # model_log.add_param(param_dict=vars(args), param_type='py_param')
    # model = model_class(args, embedd=embedded_pretrain, char_embedd=char_embedded_pretrain)
